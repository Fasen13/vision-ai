...
try {
    ...
    console.error('Не найден config.json. Создайте файл рядом с server.js.');
    process.exit(1);
}
...
const API_KEY = process.env.API_KEY || config.apiKey;if (!API_KEY) {
    console.error('Укажите apiKey в config.json или переменную окружения API_KEY.');
    process.exit(1);
}
...
async function callLLM(messages) {
    ...
    // Сначала пробуем OpenAI-совместимый вариант: `Bearer <key>`.
    // Если прокси ожидает другой формат, при 401 попробуем `<key>` без Bearer.
    ...
    return { status: result.status, json: parsed };
}
